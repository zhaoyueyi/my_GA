...
class MyGA:
    ...
    def reload_fitness_func(self, range_gene: Tuple[float, float], fitness_func, num_gene):
    ...
    def __crossover(self, parents, num_parents, prob_crossover, num_gene):
        random_prob = np.random.uniform(low=0, high=1, size=int(num_parents / 2))
        random_pos = np.random.randint(low=0, high=num_gene, size=int(num_parents / 2))  ##################
    ...
    def __mutate(self, population, prob_mutate, num_gene, range_gene):
        random_prob = np.random.uniform(low=0, high=1, size=len(population))
        random_pos = np.random.randint(low=0, high=num_gene, size=len(population))
        random_value = np.random.uniform(low=range_gene[0], high=range_gene[1], size=len(population))
        for idx, solution in enumerate(population):  # [xx, xx]
            if random_prob[idx] < prob_mutate:
                solution[random_pos[idx]] = random_value[idx]
                self.population[idx] = np.asarray(solution)
    ...
    def run(self):
        ...
        for _ in range(self.num_generation):
            ...
            self.__crossover(self.parents, self.num_parents, self.prob_crossover, self.num_gene)
            self.__mutate(self.population, self.prob_mutate, self.num_gene, self.range_gene)
